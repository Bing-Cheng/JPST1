...
load([rat '-dateW.mat']);
dateN = size(dateW,1);
sectionTitle = {'-learning stage', '-learned stage'},
...
for mi = 1: 1
    ...
    for di= 1:1
       ...
       for j=241:241
        ...
        yi = i-j+1;
        plot(IsTrail1,yi,'b.');
         plot(IsTrail2,yi+0.2,'g.');
         if ~isempty(Icoinci)
         plot(Icoinci,yi+0.1,'rs');
         else
             tmp=1;
         end
        ...
        ylabel('trial number')
xlabel('time (ms)')
        titleName = ['raster-' rat sectionT];
        title(titleName);
        saveas(h,titleName,'tif');
