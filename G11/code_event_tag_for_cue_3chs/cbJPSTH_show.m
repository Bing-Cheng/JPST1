function cbJPSTH_show(Version, matrix, xHist, yHist, xcorrHist, scoopHist, saveF,disRange)
...
%  %  toprg =max(vec2)
%  toprg = 42.1954;
%  disRange = [mean1+1.5*variance1,toprg];
...
%%%%%cb added on 08/25/11 for display
thisto1_min = min(thisto1);
thisto1_max = max(thisto1);
thisto1(1:500) = (thisto1_min*7 + thisto1_max)/8;
thisto1(end-499:end) = thisto1(1);
%%%%%cb end
