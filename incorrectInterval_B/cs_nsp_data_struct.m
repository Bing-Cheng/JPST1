...
for channel_ind = 1:length(channel)
    for date_ind = 1:length(date)
        ...
        fnamel = [direct 'ESig_' rat '-' date{date_ind} ...
                  '-A_channel' channel{channel_ind} ...
                  '-sorted-event-whole-template.mat'];
        [cTrL1 Tl] = cs_nsp_read_trail(fnamel, ts, te, binsize);
        %ESig_W5L-07-01-09-A_channel2-sorted-event-whole-template.mat
...
%         [cTrR1 Tr] = cs_nsp_read_trail(fnamer, ts, te, binsize);
         ...
         cTrR1 =0;
             Tr= 0;%put whole data in the left trial, the right trial is not used
        % form the struct
        sT = struct('left', Tl,...
                    'right', Tr,...
                    'rat', rat,...
